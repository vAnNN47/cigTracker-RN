...
references; `context/*.md` still matching reality — **incl. `context/features/*.md` must be *context
+ gotchas only*: a leftover `## Done`/`## Fix log` done-list is drift (keep the durable gotchas, move
dated "shipped X" lines to the monthly archive `context/archive/YYYY/MM-month.md`)**; `.env` covering
every var the code reads (never print values). Report as a numbered list → ask which to fix (`1,3` / `all` / `none`) → fix only
