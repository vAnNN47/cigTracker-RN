...
- **Tall sheets must set `scrollable` on `KeyboardSheet`** (e.g. `AddSmokeSheet` once "more details" is expanded) — without it the bottom-anchored sheet grows upward under the status bar with no scroll, trapping the top fields (location tags). `scrollable` caps the body below the top inset + scrolls it; in that mode only the **grabber** drags-to-dismiss (the body scrolls), so don't expect drag-from-body or drag-from-input to dismiss there — backdrop tap still does.
