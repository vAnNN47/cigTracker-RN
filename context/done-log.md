...
- Gate: `tsc` + `lint` clean; all 30 novel utility classes compile-checked through the real `@tailwindcss/postcss` → react-native-css pipeline (incl. `ms-*` → `marginInlineStart`, confirmed RN-0.85-supported + RTL-aware). 17/20 screens now done; 3 special (`progress` SVG / `index` Animated / `calendar` grid) left for solo runs. Device-eyeball pending.
