...
- Skills-system hardening (2026-06-25, from `/inbox` feedback): moved recurring rules into the **auto-loaded** standards docs so they apply globally with no per-run re-asking. `coding-standards.md` now owns **"LSP over grep"** (STOP if LSP down, grep only for raw text) + **"Verification = tsc+lint; device checks are the user's job"** + "JSDoc as you write". `ai-interaction.md` verify step reworded (device = user, not the skill). `SKILLS_README.md` house rules now point to `coding-standards.md` as canonical. **`/fire`** enforces a one-line JSDoc on every new export + dropped device-test nagging. **`/polish` `check`** now *files structural findings into the roadmap as `- [ ]` tech-debt TODOs* (was report-only) and scans via LSP; **`run`** cuts its own branch and **auto-commits without asking** (removed the "commit if asked" contradiction). No app-code change.
