...
- **`src/theme/`** — central design tokens (`useColors()`, `fonts`, spacing) consumed via React Native `StyleSheet`. **No Tailwind/NativeWind.** (Note: a second token file, [constants/theme.ts](src/constants/theme.ts), currently coexists with [theme/index.ts](src/theme/index.ts); consolidation is in progress.)
